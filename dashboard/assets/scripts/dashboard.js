...
Job.prototype = {
	fillDownloadCountBucket: function() {
		...
		this.downloadCountBucket[new Date().getSeconds()] = newDownloads;
	}
	...
	,drawPendingLogLines: function() {
		...
		while(_g < _g1.length) {
			...
			if(logLine.responseCode > 0 || logLine.wgetCode != null) {
				var text = logLine.responseCode > 0?"" + logLine.responseCode + " ":"" + logLine.wgetCode + " ";
...
var Dashboard = function(hostname,maxScrollback,showNicks,drawInterval) {
	...
	var controllerArgs = ["$scope",function(scope) {
		...
		scope.filterOperator = function(job) {
			...
			if(!StringTools.startsWith(job.ident,query)) {
...
Dashboard.getQueryArgs = function() {
	var items = StringTools.replace(window.location.search,"?","").split("&");
...
Dashboard.prototype = {
	...
	,loadRecentLogs: function() {
		...
		request.open("GET","//" + this.hostname + "/logs/recent?cb=" + new Date().getTime());
